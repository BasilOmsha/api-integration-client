    ...
    <div className="bg-background text-foreground min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-8 text-center gap-6">
        ...
        <p className="text-muted-foreground text-lg max-w-xl">
          Visualize and explore Finnish open datasets in one place. Starting with Fingrid energy data. More sources coming soon.
        </p>
      ...
      <section className="border-t px-8 py-10">
        <p className="text-center text-xs text-muted-foreground mb-6 uppercase tracking-widest">Available Data Sources</p>
        <div className="flex flex-wrap justify-center gap-4 max-w-3xl mx-auto">
          {[
				{ name: 'Fingrid', description: 'Energy & power grid data', available: true },
				{ name: 'Statistics Finland', description: 'Population & demographic data', available: false },
            ...
            >
              <div className="flex items-center gap-2">
                <span className={`size-2 rounded-full ${source.available ? 'bg-green-500' : 'bg-muted-foreground/30'}`} />
                <span className="font-medium text-sm">{source.name}</span>
              </div>
              <span className="text-xs text-muted-foreground">{source.description}</span>
              {!source.available && (
                <span className="text-xs text-muted-foreground/60 mt-1">Coming soon</span>
              )}
            </div>
          ))}
        </div>
