...
export function useChartPlot({
    ...
    isLoading,
    isFullscreen
}: ChartProps & { isFullscreen: boolean }) {
    ...
    const isEmpty = isCombo ? !comboData!.some((s) => s.points.length > 0) : dataPoints.length === 0
    ...
    const comboKey =
        isCombo && comboData
            ? comboData
                  .map(
                      (s) =>
                          `${s.config.datasetId}|${s.config.label}|${s.config.unit}|${s.config.color}|${s.config.yAxis}`
                  )
                  .join(',')
            : ''
                ...
                comboData.forEach((s, i) => {
                    seriesDefs.push({
                        ...
                        value: (_self, rawValue) =>
                            rawValue != null ? `${rawValue.toFixed(2)} ${s.config.unit}` : '--'
                    })
                })
            } else {
                seriesDefs.push({
                    ...
                    value: (_self, rawValue) =>
                        rawValue != null ? `${rawValue.toFixed(2)} ${unit}` : '--'
                })
            }
                            ...
                            fill: (self: uPlot, si: number) => {
                                ...
                                return (
                                    (typeof s.stroke === 'function'
                                        ? s.stroke(self, oi)
                                        : (s.stroke as string)) || colors.primary
                                )
                            },
            ...
            ro = new ResizeObserver((entries) => {
                for (const entry of entries) {
                    ...
                    if (w > 0 && h > 0) {
                        ...
                        chart.setSize({
                            width: Math.floor(w),
                            height: Math.max(Math.floor(h) - sliderHeight, 80)
                        })
                    }
                }
            })
